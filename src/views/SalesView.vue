<template>
  <div class="min-h-screen bg-gray-900 py-8">
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
      <h1 class="text-3xl font-bold text-white mb-8">Mis Ventas</h1>
      ...
      <div v-else class="text-center py-20">
        <p class="text-xl text-gray-400">No se encontraron ventas</p>
...
const fetchMySales = async () => {
  ...
  try {
    ...
    const response = await saleService.fetchMySales({
      page,
      limit: limit.value,
    ...
    if (response.success && response.data) {
      ...
      error.value = response.message || 'Error al cargar las ventas'
      sales.value = []
      pagination.value = null
    }
  } catch (err) {
    console.error('Error fetching my sales:', err)
    error.value = 'Error inesperado al cargar las ventas'
    sales.value = []
    pagination.value = null
  } finally {
    loading.value = false
